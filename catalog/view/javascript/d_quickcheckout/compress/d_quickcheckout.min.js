...
/*
telephone.intlTelInput({autoPlaceholder:true,utilsScript:"catalog/view/javascript/d_quickcheckout/library/phoneformat/js/utils.js"});
*/
},setDateTime:function(){var that=this;$('.date',this.el).datetimepicker({pickTime:false,dateFormat:"mm/DD/YYYY",})
$('.time',this.el).datetimepicker({pickDate:false,dateFormat:"mm/DD/YYYY",})
$('.datetime',this.el).datetimepicker({pickDate:true,pickTime:true,dateFormat:"mm/DD/YYYY",})},setValidate:function(){$(this.el).validate({submitHandler:function(form){},errorPlacement:function(error,element){error.appendTo(element.closest('div[class^="col-"]'));},highlight:function(element,errorClass,validClass){$(element.form).find("#"+element.id.replace(/\./g,'\\\.')+"_input").addClass("has-error");},unhighlight:function(element,errorClass,validClass){$(element.form).find("#"+element.id.replace(/\./g,'\\\.')+"_input").removeClass("has-error");},errorClass:"text-danger",errorElement:"div"});},

validateTelephone:function(e){
/*
if($.trim($(e.currentTarget).val())){if($(e.currentTarget).intlTelInput("isValidNumber")){$(e.currentTarget).val($(e.currentTarget).intlTelInput("getNumber"));console.log($(e.currentTarget).intlTelInput("getNumber"))
...
validateField:function(e){if($(e.currentTarget).valid()){this.updateField(e);}else{if(parseInt(config.general.analytics_event)){ga('send','event',config.name,'error',e.currentTarget.name+'.'+e.currentTarget.value);}
...
var json=this.toJSON();var that=this;$.post('index.php?route=d_quickcheckout/payment_address/update',{'payment_address':json.payment_address},function(data){that.updateForm(data);},'json').error();},updateField:function(name,value){clearTimeout(qc.payment_address_waiting);this.set(name,value);var that=this;var json=this.toJSON();qc.payment_address_waiting=setTimeout(function(){$.post('index.php?route=d_quickcheckout/payment_address/update',{'payment_address':json.payment_address},function(data){that.updateForm(data);},'json').error();},500);},});qc.PaymentAddressView=qc.View.extend({initialize:function(e){this.template=e.template;qc.event.bind("update",this.update,this);this.render();},events:{'change input[type=radio].payment-address':'changeAddress','change select.country_id':'changeCountry','change select.zone_id':'zoneDelay'},template:'',fields:'',changeAccount:function(account){if(this.model.get('account')!==account){this.model.changeAccount(account);this.render();}},changeAddress:function(e){this.model.changeAddress(e.currentTarget.value);this.render();if(parseInt(config.general.analytics_event)){ga('send','event',config.name,'update','payment_address.changeAddress');}
...
if(data.shipping_address){this.model.set('shipping_address',data.shipping_address);}
...
emailVal=email.val();var that=this;$.ajax({url:"index.php?route=d_quickcheckout/field/validate_email",async:false,method:'GET',dataType:"json",data:'email='+emailVal,success:function(data){if(data!=true){valid=false
...
if(data.account&&data.account!==this.model.get('account')){this.changeAccount(data.account);}
that.forceConfirm();},render:function(){this.focusedElementId=$(':focus').attr('id');console.log('confirm:render');$(this.el).html(this.template({'model':this.model.toJSON()}));this.fields=new qc.FieldView({el:$("#confirm_form"),model:this.model,template:_.template($("#field_template").html())});this.fields.render();$('#'+this.focusedElementId).focus();},});