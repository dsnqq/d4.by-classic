	...
	public function order() {
		...
		foreach ($order_xml->Items->Item as $item) {
			...
			if ($tax_total == 0 && $this->config->get('openbay_amazonus_order_tax') > 0) {
				$tax_total = (float)$item->Totals->Price * ($this->config->get('openbay_amazonus_order_tax') / 100) / (1 + $this->config->get('openbay_amazonus_order_tax') / 100);
			}
			...
			if ($shipping_tax == 0 && $this->config->get('openbay_amazonus_order_tax') > 0) {
				$shipping_tax = (float)$item->Totals->Shipping * ($this->config->get('openbay_amazonus_order_tax') / 100) / (1 + $this->config->get('openbay_amazonus_order_tax') / 100);
			}
			...
			if ($item_gift_wrap_tax == 0 && $this->config->get('openbay_amazonus_order_tax') > 0) {
				$item_gift_wrap_tax = (float)$item->Totals->GiftWrap * ($this->config->get('openbay_amazonus_order_tax') / 100) / (1 + $this->config->get('openbay_amazonus_order_tax') / 100);
			}
