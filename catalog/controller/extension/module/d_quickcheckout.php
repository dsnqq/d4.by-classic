...
class ControllerExtensionModuleDQuickcheckout extends Controller {
    ...
    public function index() {
        $this->model_extension_module_d_quickcheckout->logWrite('ControllerExtensionModuleDQuickcheckout Start...');
        ...
        if(VERSION >= '2.2.0.0'){
            $template = 'extension/module/d_quickcheckout';
        }elseif (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/extension/module/d_quickcheckout.tpl')) {
            $template = $this->config->get('config_template') . '/template/extension/module/d_quickcheckout.tpl';
        } else {
            $template = 'default/template/extension/module/d_quickcheckout.tpl';
        }
    ...
    public function initialize(){
        ...
        if($this->customer->isLogged()){
            ...
            if(!empty($this->session->data['payment_address']['address_id']) && $this->session->data['payment_address']['address_id'] != 'new'){
                $address = $this->model_d_quickcheckout_address->getAddress($this->session->data['payment_address']['address_id']);
                if($address){
            ...
            if(!empty($this->session->data['shipping_address']['address_id']) && $this->session->data['shipping_address']['address_id'] != 'new'){
                $address = $this->model_d_quickcheckout_address->getAddress($this->session->data['shipping_address']['address_id']);
                if($address){
