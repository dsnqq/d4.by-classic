...
class ControllerProductProduct extends Controller {
    ...
    public function index() {
        ...
        if ($product_info) {
            ...
            if ($product_info['image']) {
                ...
                $data['thumb'] = $this->model_tool_image->resize("/image/placeholder.png", $this->config->get($this->config->get('config_theme') . '_image_popup_width'), $this->config->get($this->config->get('config_theme') . '_image_popup_height'));
            }
    ...
    function isBot(&$botname = ''){
        ...
        $bots = array(
            ...
            'TelegramBot', 'WhatsApp', 'Applebot', 'DuckDuckBot', 'Baiduspider',
            'Sogou web spider', 'YoudaoBot', 'SemrushBot', 'AhrefsBot', 'MJ12bot'
        );
        foreach($bots as $bot)
    ...
    public function humanView() {
        ...
        if (!$this->isLikelyHuman() && !$this->isBot($botname)) {
            return;
        }
        ...
        if (empty($this->session->data['user_id']) && empty($this->session->data['token'])) {
            ...
            $this->model_catalog_product->updateProductStatistics($product_id, $name);
        }
    }
