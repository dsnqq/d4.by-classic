<?php
class ControllerProductAuto extends Controller {
    private $error = array();
    ...
    public function index() {
        ...
        $this->document->addScript('/catalog/view/theme/d4/js/magiczoom.js');
        ...
        $product_info = $this->model_catalog_cars->getProduct($product_id);
        ...
        if ($product_info) {
            ...
            /*if($this->request->get['product_id']) {
                if (!isBot($bname)) {
                    if (empty($this->session->data['user_id']) && empty($this->session->data['token'])) {
                        $nameStroke = $product_info['ean'] . " " . $product_info['upc'] . " " . $product_info['jan'] . "/" . $product_info['isbn'] . " " . $product_info['mpn'] . "," . $product_info['sku'] . " шт., " . $product_info['sku'] . "," . $product_info['location'];
                        $this->model_catalog_cars->updateStax($this->request->get['product_id'], $nameStroke);
                    }
                }
            }*/
            ...
            if ($product_info['image']) {
                ...
                $data['thumb'] = $this->model_tool_image->resize("/image/placeholder.png", $this->config->get($this->config->get('config_theme') . '_image_thumb_width'), $this->config->get($this->config->get('config_theme') . '_image_thumb_height'));
            }

            $data['images'] = array();

            $results = $product_info['images'];

            foreach ($results as $result) {
                $data['images'][] = array(
                    'popup' => $this->model_tool_image->resize($result, $this->config->get($this->config->get('config_theme') . '_image_popup_width'), $this->config->get($this->config->get('config_theme') . '_image_popup_height'), 'product_popup'),
                );
            }
            ...
            if ($product_info['popup']) {
                $data['popup'] = $this->model_tool_image->resize($product_info['popup'], $this->config->get($this->config->get('config_theme') . '_image_popup_width'), $this->config->get($this->config->get('config_theme') . '_image_popup_height'));
            } else {
                $data['popup'] = '';
            }

            $data['thumb']       = $product_info['thumb'];
            $data['name']        = $product_info['name'];
            $data['model']        = $product_info['model'];
            $data['modelCar']        = $product_info['modelCar'];
            $data['marka']        = $product_info['marka'];
            $data['vin']        = $product_info['vin'];
            $data['country']        = $product_info['country'];
            $data['year']        = $product_info['year'];
            $data['power']        = $product_info['power'];
            $data['volume']        = $product_info['volume'];
            $data['typePower']        = $product_info['typePower'];
            $data['aboutPower']        = $product_info['aboutPower'];
            $data['body']        = $product_info['body'];
            ...
            $this->response->setOutput($this->load->view('product/car', $data));
        } else {
            $url = '';
