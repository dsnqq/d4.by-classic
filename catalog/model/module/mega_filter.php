...
class ModelModuleMegaFilter extends Model {
	...
	public function getOptionsByType( $type, $core = null ) {
		...
		/*$ftmp	= in_array( $type, array( 'weight', 'length', 'width', 'height' ) ) ?
			"ROUND( `p`.`" . $type . "` / ( SELECT `value` FROM `" . DB_PREFIX . ( $type == 'weight' ? 'weight' : 'length' ) . "_class` WHERE `" . ( $type == 'weight' ? 'weight' : 'length' ) . "_class_id` = `p`.`" . ( $type == 'weight' ? 'weight' : 'length' ) . "_class_id` LIMIT 1 ), 0 ) AS `field`" :
			"`" . $type . "` AS `field`";*/
        /*$ftmp	= in_array( $type, array( 'weight', 'length', 'width', 'height' ) ) ?
            "ROUND( `p`.`" . $type . "` / ( SELECT `value` FROM `" . DB_PREFIX . ( $type == 'weight' ? 'weight' : 'length' ) . "_class` WHERE `" . ( $type == 'weight' ? 'weight' : 'length' ) . "_class_id` = `p`.`" . ( $type == 'weight' ? 'weight' : 'length' ) . "_class_id` LIMIT 1 ), 0 ) AS `field`" :
            ($type == 'jan' ? "CAST(`" . $type . "` AS DECIMAL(10,1)) AS `field`" : "`" . $type . "` AS `field`");*/
        $ftmp	= in_array( $type, array( 'weight', 'length', 'width', 'height' ) ) ?
            "ROUND( `p`.`" . $type . "` / ( SELECT `value` FROM `" . DB_PREFIX . ( $type == 'weight' ? 'weight' : 'length' ) . "_class` WHERE `" . ( $type == 'weight' ? 'weight' : 'length' ) . "_class_id` = `p`.`" . ( $type == 'weight' ? 'weight' : 'length' ) . "_class_id` LIMIT 1 ), 0 ) AS `field`" :
            ($type == 'jan' ? "CAST(NULLIF(`" . $type . "`, '') AS DECIMAL(10,1)) AS `field`" : "`" . $type . "` AS `field`");

		if( in_array( $type, array( 'weight', 'length', 'width', 'height' ) ) ) {
			$ftmp .= ", ROUND( `p`.`" . $type . "` / ( SELECT `value` FROM `" . DB_PREFIX . ( $type == 'weight' ? 'weight' : 'length' ) . "_class` WHERE `" . ( $type == 'weight' ? 'weight' : 'length' ) . "_class_id` = `p`.`" . ( $type == 'weight' ? 'weight' : 'length' ) . "_class_id` LIMIT 1 ), 0 ) AS `field_name`";
        ...
        if (in_array($type, array('width', 'height', 'length', 'weight'))) {
            ...
            $conditions[] = "`p`.`" . $type . "` IS NOT NULL AND `p`.`" . $type . "` != ''";
        }
