...
//heading
    ...
    $_['text_edit'] = 'Edit Custom Quick Checkout settings';
...
// Heading
    ...
    $_['text_intro_general'] = 'Set up the general settings of your Ajax Quick Checkout (CQC). You can also define positions as a module.';
...
//modules
    ...
    $_['help_position_module'] = 'If you want the CQC to show up in other locations, then you can use this option. If you just want to replace the main checkout - then do not use this option and use the optiob "Replace main checkout"';
...
//social login
    $_['text_social_login_required'] = '<h4>Social Login Required</h4><p>To use social login option in CQC, you need to install the Social Login module from Dreamvention, and setup the Social Apps. To learn how to do it follow the instructions from the Social login module.</p><p>By activating the social login buttons here, you are displaying them in the Ajax Quick Checkout in the step Login</p>';
    $_['entry_socila_login_style'] = 'Select Social login style';
    $_['help_socila_login_style'] = 'Social login for CQC uses its own settigns so that you can set different sizes of login buttons for different locations on your shop. The rest of the settings is pulled from the social login module.';
...
//payment_method
    ...
    $_['callout_payment_payment_popup'] = '<h3>Turn on or off the Popup window for each payment</h3><p>Below you can turn on or off the popup window for each payment. Every payment method has its specific conditions. In some cases we advise to use a popup window for a payment that has a lot of input feilds which you do not want to update every time you edit your address fields. Since opencart requires you to update the payment block, you can use this option to make the process more user-friendly by providing a popup window with the payment step as the final step. Also in some cases you can use this option to validate the payment method to work with CQC as a compatibility solution.</p>';
...
//cart
    ...
    $_['help_cart_option_coupon'] = 'Show or hide the coupon option. Remember, if the coupon option is disabled  in the checkout totals - it will not show up in the CQC as well. You must turn it on and then select which option gets to display it.';
    $_['help_cart_option_voucher'] = 'Show or hide the voucher option. Remember, if the voucher option is disabled  in the checkout totals - it will not show up in the CQC as well. You must turn it on and then select which option gets to display it.';
    $_['help_cart_option_reward'] = 'Show or hide the reward option. Remeber, if the reward option is disabled  in the checkout totals - it will not show up in the CQC as well. Also rewards are for logged in customers. You must turn it on and then select which option gets to display it.';


//design
    $_['entry_design_theme'] = 'Select theme for CQC';
    ...
    $_['help_design_theme'] = 'Select a style for the frontend of CQC. You can install other designs or create your own.';
