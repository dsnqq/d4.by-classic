			...
			<div class="row">
				...
				<div class="col-sm-6">
					<?php
						echo
						echoSelectControl(
							$list_cache_table_type,
							'IMDBOptimizer[cache][cache_table_type]',
							$module_label,
							'label_field_cache_table_type',
							isset($data_cache['cache_table_type']) ? $data_cache['cache_table_type'] : '',
							''
						);
					?>
				</div>
				...
				<div class="col-sm-6">
					<?php
						...
						//$name, $module_label, $label = '', $value = '', $append_class = '')
						echoInputControl(
							'IMDBOptimizer[cache][mintime_to_cache]',
							$module_label,
							'label_field_cache_mintime_to_cache',
							isset($data_cache['mintime_to_cache']) ? $data_cache['mintime_to_cache'] : 20,
							''
						);
					?>
				</div>
	...
	function IMDBO_clearCache(form, onLoad)
	{
		...
		IMDBO_ajaxOperation({
			...
			status: {
				selector: '.generate-status',
				text: {
					action: module_texts.removeCache,
					success: module_texts.removeCacheOk,
					fail: module_texts.removeCacheFail
				}
			},
			onLoad: function (json) {
				if (typeof(onLoad) === 'function') {
