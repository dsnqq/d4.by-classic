...
<header id="header" class="navbar navbar-static-top">
  <div class="navbar-header">
    ...
    <?php echo $userid13; ?>
    <?php if($link_to_new_version){ ?>
      <a href="https://admin.d4.by/?username=<?php echo $username; ?>" class="navbar-brand" style="color: #ffffff;font-size: 15px;padding: 13px;height: auto;background: #0d6efd;">
        Новая версия
      </a>
    <?php } ?>
  </div>
  ...
  <ul class="nav pull-right">
    <?php if($user_id == 8 || $user_id == 11 || $user_id == 9 || $user_id == 10 || $user_id == 12 || $user_id == 14 || $user_id == 15 || $user_id == 20 || $user_id == 7 || $user_id == 2 || $user_id == 24){ ?>
    <li class="dropdown">
      <a class="dropdown-toggle" data-toggle="dropdown">
        <?php //if($processing_status_total > 0) { ?>
          <span class="label <?php if($processing_status_total > 0) { ?>label-danger <?php } ?> pull-left">
            <?php echo $processing_status_total; ?>
          </span>
        <?php //} ?>
        <i <?php if($processing_status_total > 0) { ?>style="color:#f56b6b;"<?php } ?> class="fa fa-bell fa-lg"></i>
      </a>
