...
class ControllerModuleMegaFilter extends Controller {
	...
	private $_version		= '2.0.5.6.5.7';
	
	private $_plusRequired	= '1.3.4';
	...
	public function __construct($registry) {
		...
		if( version_compare( $this->config->get('mfilter_version'), '2.0.5.6.5.6', '<' ) ) {
			MegaFilterActivate::make( $this )->dak();
		}
		...
		if( ( ! empty( $this->request->get['reactivate_license'] ) && $this->checkPermission() ) || ( $this->config->get( 'mfilter_license' ) && ! MegaFilterActivate::cl( $this->config->get( 'mfilter_license' ) ) ) ) {
			MegaFilterActivate::make( $this )->fail();
		}
	}
	...
	private function _init( $tab ) {
		...
		if( ( version_compare( VERSION, '2.3.0.0', '<' ) || $tab != 'license' ) && ( ! $curr_ver || version_compare( $curr_ver, $this->_version, '<' ) || $this->_isOldMFilterPlus() ) ) {
			...
			if( $curr_ver ) {
				...
				if( version_compare( $curr_ver, '2.0.5.6.5', '<' ) ) {
					$this->session->data['success'] = $this->language->get('success_updated');
					$this->session->data['success'] .= '<br /><br />' . $this->language->get('text_new_activation_data');
	...
	public function index() {
		...
		if( isset( $this->session->data['activation_key'] ) ) {
			unset( $this->session->data['activation_key'] );
		}
	...
	public function import_settings() {
		...
		$this->data['action'] = '';
		$this->data['action_form'] = $this->url->link('module/mega_filter/import_settings', 'token=' . $this->session->data['token'], 'SSL');
		$this->data['action_delete_imported_file'] = $this->url->link('module/mega_filter/import_settings', 'token=' . $this->session->data['token'] . '&del=1', 'SSL');
				...
				if( empty( $this->request->post['data'] ) ) {
					...
					foreach( $data as $type => $vals ) {
						switch( $type ) {
							...
							case 'layouts' : {
								// settings
								$this->db->query("DELETE FROM `" . DB_PREFIX . "mfilter_settings`");
								
								foreach( $vals['settings'] as $row ) {
									if( isset( $row['settings']['store_id'] ) ) {
										$row['settings']['store_id'] = $this->checkImportValues( $row['settings']['store_id'], $to_import['settings']['stores'] );
									}

									if( isset( $row['settings']['layout_id'] ) ) {
										$row['settings']['layout_id'] = $this->checkImportValues( $row['settings']['layout_id'], $to_import['settings']['layouts'] );
									}

									if( isset( $row['settings']['category_id'] ) ) {
										$row['settings']['category_id'] = $this->checkImportValues( $row['settings']['category_id'], $to_import['settings']['categories'] );
									}

									if( isset( $row['settings']['hide_category_id'] ) ) {
										$row['settings']['hide_category_id'] = $this->checkImportValues( $row['settings']['hide_category_id'], $to_import['settings']['categories'] );
									}

									if( isset( $row['settings']['customer_groups'] ) ) {
										$row['settings']['customer_groups'] = $this->checkImportValues( $row['settings']['customer_groups'], $to_import['settings']['customer_groups'] );
									}

									if( isset( $row['settings']['attribs'] ) ) {
										$row['settings']['attribs'] = $this->checkImportAttributes( $row['settings']['attribs'], $to_import['settings'] );
									}

									if( isset( $row['settings']['options'] ) ) {
										$row['settings']['options'] = $this->checkImportOther( $row['settings']['options'], $to_import['settings']['options'] );
									}

									if( isset( $row['settings']['filters'] ) ) {
										$row['settings']['filters'] = $this->checkImportOther( $row['settings']['filters'], $to_import['settings']['filters'] );
									}
									
									if( isset( $row['settings']['categories'] ) ) {
										foreach( $row['settings']['categories'] as $k2 => $v2 ) {
											if( isset( $v2['root_category_id'] ) && $v2['root_category_id'] !== '' ) {
												if( 
													isset( $to_import['settings']['categories'][$row['settings']['categories'][$k2]['root_category_id']] ) &&
													$to_import['settings']['categories'][$row['settings']['categories'][$k2]['root_category_id']] !== ''
												) {
													$row['settings']['categories'][$k2]['root_category_id'] = $to_import['settings']['categories'][$row['settings']['categories'][$k2]['root_category_id']];
												} else {
													$row['settings']['categories'][$k2]['root_category_id'] = null;
												}
											}
										}
									}
									...
									$this->db->query("
										...
										SET
											`idx` = '" . $row['idx'] . "',
											`settings` = '" . $this->db->escape( json_encode( $row['settings'] ) ) . "'
									");
								}
		...
		$this->response->setOutput( $this->load->view('module/mega_filter_import_settings.tpl', $this->data) );
	}
	...
	public function export_settings() {
		...
		$this->data['action_form'] = $this->url->link('module/mega_filter/export_settings', 'token=' . $this->session->data['token'], 'SSL');
			...
			if( ! empty( $this->request->post['data'] ) ) {
				...
				foreach( $this->request->post['data'] as $type ) {
					switch( $type ) {
						case 'layouts' : {
							$data['layouts'] = array(
								'settings' => $this->db->query("SELECT * FROM `" . DB_PREFIX . "mfilter_settings`")->rows,
							...
							foreach( $data['layouts']['settings'] as $k => $row ) {
								$data['layouts']['settings'][$k]['settings'] = $row['settings'] = json_decode( $row['settings'], true );
								
								if( ! empty( $row['settings']['customer_groups'] ) ) {
									$customer_group_ids = array_merge( $customer_group_ids, $row['settings']['customer_groups'] );
								}
								
								if( ! empty( $row['settings']['category_id'] ) ) {
									$category_ids = array_unique( array_merge( $category_ids, $row['settings']['category_id'] ) );
								}
								
								if( ! empty( $row['settings']['hide_category_id'] ) ) {
									$category_ids = array_unique( array_merge( $category_ids, $row['settings']['hide_category_id'] ) );
								}
								
								if( ! empty( $row['settings']['categories'] ) ) {
									foreach( $row['settings']['categories'] as $row2 ) {
								...
								if( ! empty( $row['settings']['attribs'] ) ) {
									foreach( $row['settings']['attribs'] as $k2 => $row2 ) {
								...
								if( ! empty( $row['settings']['options'] ) ) {
									$option_ids = array_unique( array_merge( $option_ids, array_keys( $row['settings']['options'] ) ) );
								...
								if( ! empty( $row['settings']['filters'] ) ) {
									$filter_ids = array_unique( array_merge( $filter_ids, array_keys( $row['settings']['filters'] ) ) );
							...
							if( $category_ids ) {
								foreach( $this->model_module_mega_filter->getCategoriesByIds( $category_ids ) as $row ) {
		...
		$this->response->setOutput( $this->load->view('module/mega_filter_export_settings.tpl', $this->data) );
	}
	...
	public function about() {
		...
		$this->data['action'] = '';
		$this->data['action_import_settings'] = $this->url->link('module/mega_filter/import_settings', 'token=' . $this->session->data['token'], 'SSL');
		$this->data['action_export_settings'] = $this->url->link('module/mega_filter/export_settings', 'token=' . $this->session->data['token'], 'SSL');
	...
	public function license() {
		...
		if( $this->request->server['REQUEST_METHOD'] == 'POST' ) {
			if( isset( $this->session->data['activation_key'] ) ) {
				$_POST['activation_key'] = $this->session->data['activation_key'];
			}
			...
			if( empty( $_POST['activation_key'] ) ) {
				...
				$this->_saveSettings('mfilter_license', array(
					'mfilter_license' => MegaFilterActivate::e( $_POST['activation_key'] )
				));
