...
class ControllerCommonLogin extends Controller {
	...
	public function index() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			...
			if($this->request->post['remember_me'] == 'on'){
				$rememberIn = base64_encode($this->request->post['password'] . "|" . $this->request->post['username']);
				setcookie ("loginCok", $rememberIn);
			}
		...
		if (isset($this->request->post['username']) && !$_COOKIE["loginCok"]) {
			$data['username'] = $this->request->post['username'];
		} elseif($_COOKIE["loginCok"]) {
			$dataCheck = explode("|", base64_decode($_COOKIE["loginCok"]));
			$data['username'] = $dataCheck[1];
		} else{
			$data['username'] = '';
		}

		if (isset($this->request->post['password']) && !$_COOKIE["loginCok"]) {
			$data['password'] = $this->request->post['password'];
		} elseif($_COOKIE["loginCok"]) {
			$dataCheck = explode("|", base64_decode($_COOKIE["loginCok"]));
			$data['password'] = $dataCheck[0];
		} else{
			$data['password'] = '';
		}
