...
class ControllerCommonColumnLeft extends Controller {
	public function index() {
		if (isset($this->request->get['token']) && isset($this->session->data['token']) && ($this->request->get['token'] == $this->session->data['token'])) {
			...
			$data['user_groupe__info'] = $user_info['user_group'];
			if ($user_info) {
            ...
            if($user_info['user_group'] != "QR - печать" && $user_info['user_group'] != "Бухгалтерия") {
                if ($this->user->hasPermission('access', 'user/history')) {
                    $catalog[] = array(
                        'name' => "История пользователей",
                        'href' => $this->url->link('user/history', 'token=' . $this->session->data['token'], true),
                        'children' => array()
                    );
                }
            }
            ...
            if($user_info['user_group'] != "QR - печать" && $user_info['user_group'] != "Бухгалтерия") {
                if ($this->user->hasPermission('access', 'catalog/arhive')) {
                    $catalog[] = array(
                        'name' => "Архив запчастей",
                        'href' => $this->url->link('catalog/arhive', 'token=' . $this->session->data['token'], true),
                        'children' => array()
                    );
                }
                ...
                $catalog[] = array(
                    'name' => "Архив шин",
                    'href' => $this->url->link('catalog/arhive_shiny', 'token=' . $this->session->data['token'], true),
                    'children' => array()
                );
            }
            ...
            if ($this->user->hasPermission('access', 'catalog/car')) {
                $catalog[] = array(
                    'name' => "Авто в разборе",
                    'href' => $this->url->link('catalog/car', 'token=' . $this->session->data['token'], true),
                    'children' => array()
                );
            }


            if ($this->user->hasPermission('access', 'catalog/arhive_car')) {
                $catalog[] = array(
                    'name' => "Архив авто в разборе",
                    'href' => $this->url->link('catalog/arhive_car', 'token=' . $this->session->data['token'], true),
                    'children' => array()
                );
            }
			...
			if ($this->user->hasPermission('access', 'report/product_viewed')) {
                ...
                $design[] = array(
                    'name'	   => "Статистика З/Ч (новая)",
                    'href'     => $this->url->link('report/product_statistics', 'token=' . $this->session->data['token'], true),
                    'children' => array()
                );
			}
            if ($this->user->hasPermission('access', 'report/shiny_viewed')) { 
                $design[] = array(
                    'name'	   => "Статистика Шин",
                    'href'     => $this->url->link('report/shiny_viewed', 'token=' . $this->session->data['token'], true),
                    'children' => array()
                );
            }
            ...
            if($user_info['user_group'] != "Бухгалтерия") {
