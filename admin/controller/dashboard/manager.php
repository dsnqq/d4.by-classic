...
class ControllerDashboardManager extends Controller
{
	...
	{	
		...
		$data['dashboard'] = $this->url->link('common/dashboard', $token, true);
		$data['history'] = $this->url->link('dashboard/manager/history', $token, true);
		$data['invoice'] = $this->url->link('sale/order/invoice', $token, true);
		$data['delete'] = $this->url->link('dashboard/manager/delete', $token, true);
		$data['create'] = $this->url->link('sale/order/'.(version_compare(VERSION, '2.0', '<') ? 'insert' : 'add'), $token, true);
		
		if (version_compare(VERSION, '2.0', '>=')) {
			$data['shipping'] = $this->url->link('sale/order/shipping', $token, true);
		} else {
			$data['shipping'] = "";
		}

		if (version_compare(VERSION, '2.0', '>=')) {
			if (empty($this->session->data['api_token']) || !$this->getApiSession($this->session->data['api_token'])) {
				$result = $this->apiLogin();
					
				if (!empty($result['error'])) {
					$this->session->data[$this->extension.'_error_api'] = $result['error'];
				}
			}
		}
		...
		foreach ($results as $result) {
			...
			if (in_array('view_order', $data['buttons']) && $this->user->hasPermission('access', 'sale/order')) { 
				$action[] = array(
					...
					'href' => $this->url->link('sale/order/info', $token.'&order_id='.$result['order_id'].'&return='.$return, true));
			}
			...
			if (in_array('edit_order', $data['buttons']) && $this->user->hasPermission('modify', 'sale/order')) { 
				$action[] = array(
					...
					'href' => $this->url->link('sale/order/'.(version_compare(VERSION, '2.0', '<') ? 'update' : 'edit'), $token.'&order_id='.$result['order_id'].'&return='.$return, true));
			}
			...
			foreach ($products as $product) {
				...
				if ($option['type'] != 'file') {
					...
					$option_data[] = array(
						...
						'type'  => $option['type'],
						'href'  => $this->url->link('sale/order/download', $token.'&order_id='.$result['order_id'].'&order_option_id='.$option['order_option_id'], true));	
						}
					}
				}
				...
				$items[] = array(
					...
					'href' => $this->url->link('catalog/product/'.(version_compare(VERSION, '2.0', '<') ? 'update' : 'edit'), $token.'&product_id='.$product['product_id'], true),
					'name' => $product['name'],
					'quantity' => $product['quantity'],
					'options' => $option_data);
			}
			...
			if (version_compare(VERSION, '2.1', '<')) {
				$edit_customer = ($this->user->hasPermission('modify', 'sale/customer') ? $this->url->link('sale/customer/'.(version_compare(VERSION, '2.0', '<') ? 'update' : 'edit'), $token.'&customer_id='.$result['customer_id'].'&return='.$return, true) : "");
			} else {
				$edit_customer = ($this->user->hasPermission('modify', 'customer/customer') ? $this->url->link('customer/customer/edit', $token.'&customer_id='.$result['customer_id'].'&return='.$return, true) : "");
			}
	...
	public function history()
	{
		...
		unset($this->session->data[$this->extension.'_error_api']);
		...
		if (isset($this->request->post['selected'])) {
			if ($this->user->hasPermission('modify', 'sale/order')) {
				if (empty($this->session->data[$this->extension.'_error_api'])) {
					...
					$this->session->data[$this->extension.'_error'] = $this->session->data[$this->extension.'_error_api'];
				}
			} else {
	...
	public function delete()
	{
		...
		if (isset($this->request->post['selected'])) {
			if ($this->user->hasPermission('modify', 'sale/order')) {				
				if (!isset($this->session->data[$this->extension.'_error_api'])) {
					if (version_compare(VERSION, '2.0', '<')) $this->load->model('sale/order');
	...
	public function apiLogin()
	{
		...
		if ($api_info) {
			...
			if (substr($this->getStoreURL(), 0, 5) == 'https') {
			...
			curl_setopt($curl, CURLOPT_URL, $this->getStoreURL().'index.php?route=api/login');
	...
	public function apiRequest($url, $data = array())
	{
		...
		if (isset($this->session->data['cookie']) || isset($this->session->data['api_token'])) {
			...
			if (isset($this->session->data['api_token'])) {
				...
				$session = $this->getApiSession($this->session->data['api_token']);
			} else {
	...
	{
		$query = $this->db->query("SELECT session_id FROM `".DB_PREFIX."api_session` WHERE token = '".$this->db->escape($token)."'");
