...
class ControllerCatalogAqeProduct extends Controller {
	...
	protected function getList() {
		...
		// lightbox
		$this->document->addStyle('view/stylesheet/lightbox.min.css');
		$this->document->addScript('view/stylesheet/lightbox.min.js');
		...
		foreach ($stores as $store) {
			...
			$_url = new Url($store['url'], $store['ssl']);
			...
			if (!is_array($columns)) {
				...
				foreach ($columns as $column => $attr) {
					if ($column == 'image') {
						...
						$row['etvylet'] = $result['etvylet'];
						$row['diadiametr'] = $result['diadiametr'];
		...
		if ($this->request->server['REQUEST_METHOD'] == 'POST' && $this->validateUpdateData($this->request->post)) {
			...
			if ($results['done']) {
				...
				if (in_array($column, array('model', 'sku', 'upc', 'location', 'attributes', 'discounts', 'images', 'options', 'recurrings', 'related', 'specials', 'descriptions'))) {
					...
					$response['value'] = sprintf("%.4f",round((float)$value));
					$response['values']['*'] = $response['value'];
