...
class ControllerExtensionPaymentYandexMoney extends Controller
{
    ...
    public function incl($file) {
        return modification(DIR_TEMPLATE . $this->getPrefix() . 'payment/yandex_money/' . $file);
    }
    ...
    public function index()
    {
        ...
        $data['kassa_logs_link'] = $this->url->link($prefix.'payment/'.self::MODULE_NAME.'/logs',
            'token='.$this->session->data['token'], true);
        $data['install_widget']  = $this->url->link($prefix.'payment/'.self::MODULE_NAME.'/installWidget',
            'token='.$this->session->data['token'], true);
    ...
    private function validate(Request $request)
    {
        ...
        if ($this->getModel()->getKassaModel()->isEnabled()) {
            $enabled = true;
            $request->post['yandex_money_sort_order'] = $request->post['yandex_money_kassa_sort_order'];
        } elseif ($this->getModel()->getWalletModel()->isEnabled()) {
            $enabled = true;
            $request->post['yandex_money_sort_order'] = $request->post['yandex_money_wallet_sort_order'];
        } elseif ($this->getModel()->getBillingModel()->isEnabled()) {
            $enabled = true;
            $request->post['yandex_money_sort_order'] = $request->post['yandex_money_billing_sort_order'];
        }
    ...
    private function validateKassa(Request $request)
    {
        ...
        $value = isset($request->post['yandex_money_kassa_invoice']) ? $request->post['yandex_money_kassa_invoice'] : false;
