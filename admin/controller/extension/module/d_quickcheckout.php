...
class ControllerExtensionModuleDQuickcheckout extends Controller {
    ...
    public function __construct($registry) {
        ...
        //$this->model_extension_module_d_quickcheckout->installDependencies($this->mbooth);
    }
    ...
    public function index() {
        ...
        if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
            ...
            if (isset($this->request->get['setting_id'])) {
                ...
                $this->model_extension_module_d_quickcheckout->editSetting($this->request->get['setting_id'], $this->request->post[$this->id . '_setting']);
            }

            if($this->d_shopunity){
                $this->load->model('d_shopunity/vqmod');
                if($this->request->post[$this->id . '_status']){
        ...
        } elseif ($this->model_extension_module_d_quickcheckout->getCurrentSettingId($this->id, $this->store_id)) {
            $url .= '&setting_id=' . $this->model_extension_module_d_quickcheckout->getCurrentSettingId($this->id, $this->store_id);
        }
        ...
        $data['support_email'] = $this->model_extension_module_d_quickcheckout->getMboothInfo($this->mbooth)->support_email;
        $data['version'] = $this->model_extension_module_d_quickcheckout->getVersion($data['mbooth']);
        ...
        $data['add_field'] = $this->url->link('customer/custom_field/add', 'token=' . $this->session->data['token'], true);
        ...
        $data['update'] = $this->model_extension_module_d_quickcheckout->ajax($this->url->link($this->route . '/getUpdate', 'token=' . $this->session->data['token'], true));

        //debug
        $data['tab_debug'] = $this->language->get('tab_debug');
        $data['entry_debug'] = $this->language->get('entry_debug');
        $data['entry_debug_file'] = $this->language->get('entry_debug_file');
        $data['clear_debug_file'] = $this->model_extension_module_d_quickcheckout->ajax($this->url->link($this->route . '/clearDebugFile', 'token=' . $this->session->data['token'], true));
        ...
        $data['link_social_login_edit'] = $this->url->link('extension/module/d_social_login', 'token=' . $this->session->data['token'] . '&store_id=' . $this->store_id, true);
        ...
        //get currant setting
        $data[$this->id . '_status'] = $this->model_extension_module_d_quickcheckout->getConfigData($this->id, $this->id . '_status', $this->store_id, $this->config_file);
        $data['debug'] = $this->model_extension_module_d_quickcheckout->getConfigData($this->id, $this->id . '_debug', $this->store_id, $this->config_file);
        $data['debug_file'] = $this->model_extension_module_d_quickcheckout->getConfigData($this->id, $this->id . '_debug_file', $this->store_id, $this->config_file);
        $data['setting'] = $this->model_extension_module_d_quickcheckout->getConfigSetting($this->id, $this->id . '_setting', $this->store_id, $this->config_file);
        $data[$this->id . '_trigger'] = $this->model_extension_module_d_quickcheckout->getConfigData($this->id, $this->id . '_trigger', $this->store_id, $this->config_file);
        ...
        $data['setting_id'] = $this->model_extension_module_d_quickcheckout->getCurrentSettingId($this->id, $this->store_id);
        ...
        $data['save_bulk_setting'] = $this->model_extension_module_d_quickcheckout->ajax($this->url->link($this->route . '/saveBulkSetting', 'token=' . $this->session->data['token'] . $url, true));
        ...
        //get config 
        $data['config_files'] = $this->model_extension_module_d_quickcheckout->getConfigFiles($this->id);
        ...
        foreach ($data['statistics'] as $key => $value) {
            ...
            if ($value['customer_id']) {
                $data['statistics'][$key]['href_customer'] = $this->url->link('sale/customer/edit', 'token=' . $this->session->data['token'] . '&customer_id=' . $value['customer_id'], true);
            } else {
        ...
        foreach ($data['analytics_getAll'] as $key => $value) {
            ...
            if ($value['customer_id']) {
                $data['analytics_getAll'][$key]['href_customer'] = $this->url->link('sale/customer/edit', 'token=' . $this->session->data['token'] . '&customer_id=' . $value['customer_id'], true);
            } else {
        ...
        $pagination->url = $this->url->link('extension/module/d_quickcheckout', 'token=' . $this->session->data['token'] . $url . '&page={page}', true);
    ...
    public function createSetting() {
        ...
        if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
            ...
            $setting_id = $this->model_extension_module_d_quickcheckout->setSetting($setting_name, $this->request->post[$this->id . '_setting'], $this->store_id);
        }
        $this->load->language($this->route);
        if ($setting_id) {
            $this->session->data['success'] = $this->language->get('success_setting_created');
            $json['redirect'] = $this->model_extension_module_d_quickcheckout->ajax($this->url->link($this->route, 'store_id=' . $this->store_id . '&setting_id=' . $setting_id . '&token=' . $this->session->data['token'], true));
        } else {
    ...
    public function deleteSetting() {
        ...
        if (($this->request->server['REQUEST_METHOD'] == 'GET') && $this->validate() && isset($this->request->get['setting_id'])) {
            ...
            $this->response->redirect($this->url->link($this->route, 'store_id=' . $this->store_id . '&token=' . $this->session->data['token'], true));
        } else {
            $setting_id = $this->request->get['setting_id'];
            $this->session->data['error'] = $this->language->get('error_setting_not_deleted');
            $this->response->redirect($this->url->link($this->route, 'store_id=' . $this->store_id . '&setting_id=' . $setting_id . '&token=' . $this->session->data['token'], true));
        }
    }
    ...
    public function getUpdate($status = 1) {
        ...
        $current_version = $this->model_extension_module_d_quickcheckout->getVersion($this->mbooth);
        $info = $this->model_extension_module_d_quickcheckout->getUpdateInfo($this->mbooth, $status);
