...
class ModelCatalogShiny extends Model {
    ...
    /*
     * Добавляем в БД запись в историю
     * */
    public function addChangeProductData($data_old, $data_new, $data_name, $product_id, $user_id) {
        $this->db->query("INSERT INTO " . DB_PREFIX . "shiny_change SET product_id = '" . (int)$product_id . "', value_name = '". $data_name ."', data_change = NOW(), value_old = '" . $data_old . "', value_new = '" . $data_new . "', user_id = '" . $user_id . "'");

        $this->cache->delete('shiny');
    }

    public function addImageForProduct($product_id, $image_path){
        $query = $this->db->query("SELECT * FROM " . DB_PREFIX . "shiny WHERE product_id = '" . (int)$product_id . "'");
        $main_image = $query->row;

        // Проверяем есть ли главное изображении у запчасти
        if($main_image['image'] != "" && $main_image['image'] != NULL){
            if (isset($image_path)) {
                $this->db->query("INSERT INTO " . DB_PREFIX . "shiny_image SET product_id = '" . (int)$product_id . "', image = '" . $this->db->escape($image_path) . "', sort_order = '0'");
            }
        } else{
            if (isset($image_path)) {
                $this->db->query("UPDATE " . DB_PREFIX . "shiny SET image = '" . $this->db->escape($image_path) . "' WHERE product_id = '" . (int)$product_id . "'");
                $this->db->query("INSERT INTO " . DB_PREFIX . "shiny_image SET product_id = '" . (int)$product_id . "', image = '" . $this->db->escape($image_path) . "', sort_order = '0'");
            }
        }
	...
	public function editProduct($product_id, $data) {
		...
		$this->db->query("UPDATE " . DB_PREFIX . "shiny SET model = '" . $this->db->escape($data['model']) . "', sku = '" . $this->db->escape($data['sku']) . "', upc = '" . $this->db->escape($data['upc']) . "', ean = '" . $this->db->escape($data['ean']) . "', jan = '" . $this->db->escape($data['jan']) . "', diadiametr = '" . $this->db->escape($data['diadiametr']) . "', version = '" . $this->db->escape($data['version']) . "', etvylet = '" . $this->db->escape($data['etvylet']) . "', isbn = '" . $this->db->escape($data['isbn']) . "', mpn = '" . $this->db->escape($data['mpn']) . "', location = '" . $this->db->escape($data['location']) . "', quantity = '" . (int)$data['quantity'] . "', minimum = '" . (int)$data['minimum'] . "', subtract = '" . (int)$data['subtract'] . "', stock_status_id = '" . (int)$data['stock_status_id'] . "', date_available = '" . $this->db->escape($data['date_available']) . "', manufacturer_id = '" . (int)$data['manufacturer_id'] . "', shipping = '" . (int)$data['shipping'] . "', price = '" . (float)$data['price'] . "', points = '" . (int)$data['points'] . "', weight = '" . (float)$data['weight'] . "', weight_class_id = '" . (int)$data['weight_class_id'] . "', length = '" . (float)$data['length'] . "', width = '" . (float)$data['width'] . "', height = '" . (float)$data['height'] . "', length_class_id = '" . (int)$data['length_class_id'] . "', status = '" . (int)$data['status'] . "', tax_class_id = '" . (int)$data['tax_class_id'] . "', sort_order = '" . (int)$data['sort_order'] . "', date_modified = NOW() WHERE product_id = '" . (int)$product_id . "'");
	...
	public function copyProduct($product_id) {
		...
		if ($query->num_rows) {
			...
			$data['shiny_description'] = $this->getProductDescriptions($product_id);
	...
	public function getProduct($product_id) {
		$query = $this->db->query("SELECT DISTINCT *, (SELECT keyword FROM " . DB_PREFIX . "url_alias WHERE query = 'product_id=" . (int)$product_id . "' LIMIT 1) AS keyword FROM " . DB_PREFIX . "shiny WHERE product_id = '" . (int)$product_id . "' ");
	...
	public function getProducts($data = array()) {
			...
			$sql = "SELECT * FROM " . DB_PREFIX . "shiny WHERE 1=1";
...
*/
	...
	public function getProductsByCategoryId($category_id) {
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "product p LEFT JOIN " . DB_PREFIX . "shiny_description pd ON (p.product_id = pd.product_id) LEFT JOIN " . DB_PREFIX . "product_to_category p2c ON (p.product_id = p2c.product_id) WHERE pd.language_id = '" . (int)$this->config->get('config_language_id') . "' AND p2c.category_id = '" . (int)$category_id . "' ORDER BY pd.name ASC");

		return $query->rows;
	}

	public function getProductDescriptions($product_id) {
		$shiny_description_data = array();

		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "shiny_description WHERE product_id = '" . (int)$product_id . "'");

		foreach ($query->rows as $result) {
			$shiny_description_data[$result['language_id']] = array(
				'name'             => $result['name'],
	...
	public function getProductCountStatusNo($data = array()){
		$query = $this->db->query("SELECT COUNT(*) AS total FROM " . DB_PREFIX . "shiny WHERE status = '0'");
    ...
    public function getChangeProductData($product_id) {
        $changeProductData = $this->db->query("SELECT * FROM " . DB_PREFIX . "shiny_change p LEFT JOIN " . DB_PREFIX . "user pd ON (p.user_id = pd.user_id)  WHERE product_id = '" . (int)$product_id . "'");
