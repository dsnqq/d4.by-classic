<?php 
    ...
    while($result_item = mysqli_fetch_array($result)){
        ...
        /*
        $images_product = '';
        $link_product = "https://d4.by/index.php?route=product/product&amp;product_id=" . $result_item['product_id']. '<br>';
        $html_content = file_get_contents( $link_product );
        $html = new simple_html_dom();
        $html->load( $html_content );
        $element = $html->find( ".wrap_thumb_img_pars" );
        foreach($element as $element__item){
            preg_match_all('/src=\"(.*?)\"/', $element__item, $matches);
            if($matches[1][0]){
                $images_product .= $matches[1][0].',';
            } else{
                $images_product .= '';
            }
        }
