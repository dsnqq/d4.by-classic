...
class MegaFilterCore {	
	...
	static public function prepareSeoParts( & $ctrl, $parts ) {
		...
		if( null != ( $joinParts = implode( '/', $parts ) ) && preg_match( '#/?'.$mfp_seo_sep.'/([a-z0-9\-_]+,[^/]+/?)+#', $joinParts, $matches ) ) {
			if( isset( $ctrl->request->get['route'] ) ) {
				$ctrl->request->get['route'] = preg_replace( '#/?'.$mfp_seo_sep.'/([a-z0-9\-_]+,[^/]+/?)+#', '', $ctrl->request->get['route'] );
			}
		
			if( isset( $ctrl->request->get['_route_'] ) ) {
				$ctrl->request->get['_route_'] = preg_replace( '#/?'.$mfp_seo_sep.'/([a-z0-9\-_]+,[^/]+/?)+#', '', $ctrl->request->get['_route_'] );
			}
			...
			$joinParts = preg_replace( '#/?'.$mfp_seo_sep.'/([a-z0-9\-_]+,[^/]+/?)+#', '', $joinParts );
		...
		if( ! empty( $seo_settings['auto_redirect_to_seo_aliases'] ) && ! empty( $ctrl->request->get[$mfp_url_param] ) && empty( $ctrl->request->get['mfilterAjax'] ) ) {
			...
			if( ! empty( $params[1] ) && ! empty( $params[2] ) ) {
				...
				$query = $ctrl->db->query("
					...
					WHERE
						`path` = '" . ( $smp_language ? $smp_language . '/' : '' ) . implode('/', $parts) . "' AND
						`mfp` = '" . $ctrl->db->escape( implode( ',', $temp ) ) . "' AND
	...
	static public function prepareSeoPart( & $ctrl, $part ) {
		...
		if( preg_match( '/^'.$mfp_seo_sep.',([a-z0-9\-_]+\[[^\]]*\],?)+/', $part, $matches ) ) {
			if( isset( $ctrl->request->get['route'] ) ) {
				$ctrl->request->get['route'] = preg_replace( '/\/?'.$mfp_seo_sep.',([a-z0-9\-_]+\[[^\]]*\],?)+/', '', $ctrl->request->get['route'] );
			}
	...
	static public function preparePath( & $ctrl, $path ) {
		if( $ctrl->config->get('smp_version') ) {
			...
			if( $ctrl->config->get( 'smp_add_default_language_code_to_url' ) || $smp_default_language != $ctrl->config->get('config_language') ) {
				if( ! empty( $ctrl->session->data['language'] ) ) {
					$smp_language = $ctrl->session->data['language'];
				} else if( ! empty( $_COOKIE['language'] ) ) {
					$smp_language = htmlspecialchars($_COOKIE['language'], ENT_COMPAT, 'UTF-8');
	...
	public function getJsonData( array $types, $idx = NULL ) {
		...
		foreach( $types as $type ) {
			if( in_array( $type, array( 'manufacturers', 'stock_status', 'rating', 'price', 'discounts' ) ) ) {
				switch( $type ) {
				...
				switch( $type ) {
					...
					case 'categories:cat_checkbox' : {
						$json[$type] = $this->getTreeCategories( $idx, null, 'checkbox' );
						
						break;
					}
					case 'categories:tree'	: {
						$json[$type] = $this->getTreeCategories( $idx, null, 'tree' );
					...
					case 'vehicles'			: {
						...
						foreach( $this->_ctrl->model_module_mega_filter->vehiclesToJson( $idx, $this, array() ) as $k => $v ) {
							$json['vehicles'][$k] = $v;
						}
					...
					case 'levels'			: {
						...
						foreach( $this->_ctrl->model_module_mega_filter->levelsToJson( $idx, $this, array() ) as $k => $v ) {
							$json['levels'][$k] = $v;
						}
		...
		if( 
			...
			$sql = $this->_ctrl->model_module_mega_filter->createQuery( $sql, array(
	...
	private function __construct( & $ctrl, $sql, array $data = array(), array $settings = array() ) {		
		...
		if( self::$_om === null ) {
			self::$_om = $this->_ctrl->model_module_mega_filter->iom() ? true : false;
		}
	...
	protected function findSettings( $settings ) {
		...
		if( isset( $this->_ctrl->request->post['mfilterIdx'] ) ) {
			$config = $this->_ctrl->model_module_mega_filter->getModuleSettings( $this->_ctrl->request->post['mfilterIdx'] );
		} else if( isset( $this->_ctrl->request->get['mfilterIdx'] ) ) {
			$config = $this->_ctrl->model_module_mega_filter->getModuleSettings( $this->_ctrl->request->get['mfilterIdx'] );
		} else {
			foreach( $this->_ctrl->db->query("SELECT * FROM `" . DB_PREFIX . "layout_module` WHERE `layout_id` = '" . (int)$layout_id . "' AND `code` LIKE 'mega_filter%' ORDER BY `sort_order`")->rows as $module ) {
				...
				if( isset( $part[1] ) ) {
					$config = $this->_ctrl->model_module_mega_filter->getModuleSettings( $part[1] );
	...
	public function addParamToCurrentUrl( array $data, $param_name, $param_value ) {
		...
		if( $route == 'common/home' ) {
			$route = 'module/mega_filter/results';
		}
	...
	public function getCurrentPath() {
		...
		if( $this->rget('path') !== null ) {
			if( ! isset( self::$_cache['paths'][$this->rget('path')] ) ) {
				...
				foreach ($categories as $category_id) {
					$query = $this->_ctrl->db->query("SELECT * FROM " . DB_PREFIX . "url_alias WHERE `query` = 'category_id=" . (int)$category_id . "'");
		...
		if( ! isset( self::$_cache['aliases'][$path] ) ) {
			...
			foreach( $alias = $this->_ctrl->db->query( "
				...
				WHERE
					...
					( `path` = '' OR `path` = '" . $this->_ctrl->db->escape( $path ) . "' OR `path` = '" . $this->_ctrl->db->escape( $this->_ctrl->config->get('config_language') . '/' . $path ) . "' )
			")->rows as $row ) {
	...
	public function cacheName() {
		return md5( 
			...
			serialize( $this->_ctrl->model_module_mega_filter->prepareBaseConditions(array()) ) .
	...
	public static function __parseParams( $mfp ) {
		preg_match_all( '/([a-z0-9\-_]+)\[([^\]]*)\]/', $mfp, $matches );
			...
			if( ! empty( $matches[0] ) ) {
				foreach( $matches[0] as $k => $match ) {
					...
					switch( $key ) {
						...
						case 'path' : {
							if( isset( $value[0] ) ) {
								if( count( $value ) == 1 && strpos( $value[0], '_' ) !== false ) {
									$value[0] = explode( '_', $value[0] );
									$value[0] = end( $value[0] );
								}
								
								if( ! empty( $this->_data['mfp_overwrite_path'] ) ) {
									$this->_data['path'] = implode( ',', $this->aliasesToIds( 'category_id', explode( ',', $this->parsePath( $value ) ) ) );
	...
	private static function createAliases( & $ctrl, $type, $column, $values ) {
		...
		if( $values ) {
			$sql = "SELECT * FROM `" . DB_PREFIX . "url_alias` AS `ua` WHERE `" . $column . "` IN(" . implode( ',', $values ) . ")";

			if( $ctrl->config->get( 'smp_is_install' ) ) {
				$sql .= " AND `ua`.`smp_language_id` = '" . (int) $ctrl->config->get('config_language_id') . "'";
			}
	...
	private function _mfPriceCol( $alias = 'mf_price' ) {
		if( self::$_om && $this->_ctrl->model_module_mega_filter->iom( 'mfPriceCol' ) ) {
			return $this->_ctrl->model_module_mega_filter->om( 'mfPriceCol', $this, func_get_args() );
		}
	...
	public function getSQL( $fn, $sql = NULL, $template = NULL, array $conditions = array() ) {
		...
		if( 
			! empty( $this->_parseParams['vehicle_make_id'] ) 
				|| 
			! empty( $this->_parseParams['levels'] ) 
				|| 
			! empty( $this->_data['filter_category_id'] ) 
				|| 
			! empty( $conditions['in']['search'] ) 
				|| 
			( empty( $this->_data['filter_category_id'] ) && ! empty( $this->_parseParams['path'] ) )
		) {
			$skip = array();
		...
		if( ! empty( $this->_parseParams['path'] ) ) {
			$category_ids = implode( ',', $this->parseArrayToInt( $this->aliasesToIds('category_id', $this->_parseParams['path']) ) );
		}
		
		if( $category_ids && ! empty( $this->_data['filter_category_id'] ) ) {
			$sql = preg_replace( '/AND\s+`?p2c`?\.`?category_id`?\s*=\s*(\'|")[0-9]+(\'|")/ims', 'AND `p2c`.`category_id` IN(' . $category_ids . ')', $sql );
			$sql = preg_replace( '/AND\s+`?cp`?\.`?path_id`?\s*=\s*(\'|")[0-9]+(\'|")/ims', 'AND `cp`.`path_id` IN(' . $category_ids . ')', $sql );
		}
	...
	private function _attribsToSQL( $join = ' WHERE ', array $attribs = NULL, & $conditionsIn = NULL, & $conditionsOut = NULL, $field_id = '`product_id`' ) {
		...
		if( $attribs ) {
			...
			foreach( $attribs as $key => $attr ) {
				...
				if( null != ( $attr = array_filter( $attr ) ) ) {
					$sql[]	= sprintf( $field_id . " IN( 
						SELECT 
							`product_id` 
						FROM 
	...
	private function _ratingCol( $alias = 'mf_rating' ) {
		if( self::$_om && $this->_ctrl->model_module_mega_filter->iom( 'ratingCol' ) ) {
			return $this->_ctrl->model_module_mega_filter->om( 'ratingCol', $this, func_get_args() );
		}
	...
	public function discountCol( $alias = 'discount' ) {
		if( self::$_om && $this->_ctrl->model_module_mega_filter->iom( 'discountCol' ) ) {
			return $this->_ctrl->model_module_mega_filter->om( 'discountCol', $this, func_get_args() );
		}
	...
	public function specialCol( $alias = 'special' ) {
		if( self::$_om && $this->_ctrl->model_module_mega_filter->iom( 'specialCol' ) ) {
			return $this->_ctrl->model_module_mega_filter->om( 'specialCol', $this, func_get_args() );
		}
	...
	private function _taxCol( $type, $alias ) {
		if( self::$_om && $this->_ctrl->model_module_mega_filter->iom( 'taxCol' ) ) {
			return $this->_ctrl->model_module_mega_filter->om( 'taxCol', $this, func_get_args() );
		}
	...
	public function priceCol( $alias = 'price' ) {
		if( self::$_om && $this->_ctrl->model_module_mega_filter->iom( 'priceCol' ) ) {
			return $this->_ctrl->model_module_mega_filter->om( 'priceCol', $this, func_get_args() );
		}
	...
	public function _baseConditions( array $conditions = array(), $internal = false ) {
		...
		if( ! empty( $data['filter_name'] ) && $this->_ctrl->config->get( 'mfilter_search_enabled' ) ) {
			if( ! class_exists( 'Mfilter_Search' ) ) {
				if( class_exists( '\VQMod' ) ) {
					require_once \VQMod::modCheck(\VQMod::modCheck(modification(DIR_SYSTEM . 'library/mfilter_search.php')));
				} else {
					require_once modification(DIR_SYSTEM . 'library/mfilter_search.php');
				}
			}
					
			if( class_exists( 'Mfilter_Search' ) && $this->_ctrl->config->get( 'mfilter_search_version' ) && $this->_ctrl->config->get( 'mfilter_search_license' ) ) {
				$mfilter_search = true;
			}
		}
		
		if( empty( $data['filter_category_id'] ) && ! empty( $this->_parseParams['path'] ) ) {
			$data['filter_category_id'] = implode( ',', $this->aliasesToIds('category_id', $this->_parseParams['path']) );
		}
		
		$category_ids = null;
		
		if( ! empty( $this->_parseParams['path'] ) ) {
			$category_ids = implode( ',', $this->aliasesToIds( 'category_id', $this->_parseParams['path'] ) );
		} else if( ! empty( $data['filter_category_id'] ) ) {
			$category_ids = implode( ',', $this->parseArrayToInt( explode( ',', $data['filter_category_id'] ) ) );
		}
		
		if( $category_ids ) {
			if( ! empty( $data['filter_sub_category'] ) || $this->_categories ) {
				if( ! empty( $this->_settings['try_to_boost_subcategories_speed'] ) ) {
					$conditions['cat_id'] = "`p2c`.`category_id` IN(SELECT `category_id` FROM `" . DB_PREFIX . "category_path` WHERE `path_id` IN(" . $category_ids . "))";
				} else {
					$conditions['cat_id'] = "`cp`.`path_id` IN(" . $category_ids . ")";
				}
			} else {
				$conditions['cat_id'] = "`p2c`.`category_id` IN(" . $category_ids . ")";
			}
		...
		return $this->_ctrl->model_module_mega_filter->prepareBaseConditions( $conditions );
	}
	...
	public function _baseJoin( array $skip = array(), $mainQuery = false ) {
		...
		if( ! empty( $this->_data['filter_category_id'] ) || $this->_categories || ! empty( $this->_parseParams['path'] ) ) {
			if( ! in_array( 'p2c', $skip ) ) {
				$sql['p2c'] = $this->_joinProductToCategory( 'p2c' );
			}
			
			if( ( ! empty( $this->_data['filter_sub_category'] ) || $this->_categories || ! empty( $this->_parseParams['path'] ) ) && ! in_array( 'cp', $skip ) ) {
				if( empty( $this->_settings['try_to_boost_subcategories_speed'] ) ) {
		...
		return implode( '', $this->_ctrl->model_module_mega_filter->prepareBaseJoin( $sql, $skip, $mainQuery ) );
	}
	
	public function _joinVehicle( $skipData = false, $mainQuery = false ) {
		if( ! $this->_ctrl->model_module_mega_filter->hasVehicles() ) {
			return '';
		}
	...
	public function _joinLevel( $skipData = false ) {
		if( ! $this->_ctrl->model_module_mega_filter->hasLevels() ) {
			return '';
		}
	...
	public function getCurrencyValue() {
		if( self::$_om && $this->_ctrl->model_module_mega_filter->iom( 'getCurrencyValue' ) ) {
			return $this->_ctrl->model_module_mega_filter->om( 'getCurrencyValue', $this, func_get_args() );
		}
	...
	public function getTreeCategories( $idx, $root_cat = NULL, $type = null ) {
		$settings = array();
		
		if( $type && in_array( $type, array( 'tree' ) ) ) {
			$settings = $this->_ctrl->model_module_mega_filter->getModuleSettings( $idx );

			if( ! empty( $settings['categories'] ) ) {
				foreach( $settings['categories'] as $v ) {
					if( $v['type'] == $type ) {
						$settings = $v;
		...
		$sql		= "
			SELECT
				...
				`c`.`category_id`," . (
					empty( $this->_seo_settings['enabled'] ) ? '' :
					"(
						SELECT `keyword` FROM `" . DB_PREFIX . "url_alias` AS `ua` WHERE `query` = CONCAT( 'category_id=', `c`.`category_id` ) " . (
							$this->_ctrl->config->get( 'smp_is_install' ) ? "
								AND `ua`.`smp_language_id` = '" . (int) $this->_ctrl->config->get('config_language_id') . "'
							" : ''
					) . " LIMIT 1) AS `keyword`,"
				) . ( ! empty( $settings['generate_normal_links_instead_of_ajax_request'] ) ? "
				IF( `c`.`parent_id` = 0, `c`.`category_id`, (
					SELECT
						GROUP_CONCAT( `path_id` ORDER BY `level` SEPARATOR '_' )
					FROM
		...
		$sql = $this->_ctrl->model_module_mega_filter->createQuery( $sql, array(), 'getTreeCategories_main');
		...
		foreach( $this->_ctrl->db->query( $sql )->rows as $cat ) {
			...
			if( isset( $cat['path'] ) ) {
				...
				if( ! empty( $settings['generate_normal_links_instead_of_ajax_request'] ) ) {
					$row['link'] = $this->_ctrl->url->link( 'product/category', '&path=' . $row['path'], 'SSL' );
				}
			}
					
			self::$_cache[__METHOD__][$root_cat][] = $row;
		}
		...
		if( $cat_ids ) {
			...
			$sql = $this->_ctrl->model_module_mega_filter->createQuery( $sql, array(), 'getTreeCategories_counts');
	...
	public function getCountsByTags() {
		...
		$sql = $this->_ctrl->model_module_mega_filter->createQuery( $sql, array(), __FUNCTION__ );
	...
	public function getCountsByBaseType( $type ) {		
		...
		if( in_array( $this->route(), MegaFilterCore::$_specialRoute ) ) {
			$conditionsOut[] = '(' . $this->specialCol( '' ) . ') IS NOT NULL';
		}
	...
	private function _getCountsByAttributes( array $conditions, array $conditionsIn ) {
		...
		$sql = $this->_ctrl->model_module_mega_filter->createQuery( $sql, array(), 'attrCount' );
	...
	private function _getCountsByOptions( array $conditions, array $conditionsIn ) {
		...
		$sql = $this->_ctrl->model_module_mega_filter->createQuery( $sql, array(), 'optsCount' );
	...
	private function _getCountsByFilters( array $conditions, array $conditionsIn ) {
		...
		$sql = $this->_ctrl->model_module_mega_filter->createQuery( $sql, array(), 'filterCount' );
